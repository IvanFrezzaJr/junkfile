...
def copy_to_organized_directory(dir_src: str, dir_dst: str) -> bool:
    ...
    if not dir_src or not dir_dst:
        raise OSError("dir_src or dir_dst are empty")
    ...
    for file in list_files:
        ...
        if search:
            ...
            if not checks.check_exists(directory_extension):
                if not tools.create_directory(directory_extension):
                    raise OSError("Directory '{directory_extension}' could not be created.")
...
def current_directory_handler(dir_dst: str):
    ...
    if not dir_dst:
        raise ValueError("dir_dst is empty")


    if not os.path.exists(dir_dst):
        raise ValueError("dir_dst not exists. dir_dst: '{dir_dst}'")


    if not checks.check_permission(dir_dst):
        raise IOError("You don't have permition to access that folder: {0}".format(dir_dst))
...
def build_directory_path_with_date(path_folder: str) -> str:
    ...
    if not checks.check_exists(path_folder):
        raise IOError("path_folder not found. path_folder: '{path_folder}'")
...
def run(path_folder, create_copy=True):
    ...
    with current_directory_handler(path_folder):
        ...
        if not new_directory:
            raise IOError(f"new_directory could not be builded. new_directory: '{new_directory}'")
